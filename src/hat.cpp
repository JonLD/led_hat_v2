...
int wave_flash_double[] = {
    wave_whole, clear_display, flash_whole, clear_display, wave_whole,
...
void wave_effect(int note_length)
{
    if (!radioData.shouldAttemptResync)
    {
        ...
        int y = MAX_Y_INDEX;
        while (
            ((currentTime_ms - startOfBeatTime_ms) <
             (radioData.beatLength_ms) / note_length) &&
...
void flash_and_fade(int note_length)
{
    if (!radioData.shouldAttemptResync)
    {
        unsigned long startOfBeatTime_ms = millis();
        ...
        FastLED.show();
        while (((currentTime_ms - startOfBeatTime_ms) <
                ((radioData.beatLength_ms) / note_length)) &&
               (!radioData.shouldAttemptResync))
        {
            EVERY_N_MILLIS(5)
...
void twinkle_shaker(int note_length)
{
    if (!radioData.shouldAttemptResync)
    {
        unsigned long startOfBeatTime_ms = millis();
        unsigned long currentTime_ms = startOfBeatTime_ms;
        while (((currentTime_ms - startOfBeatTime_ms) < radioData.beatLength_ms / note_length) &&
               (!radioData.shouldAttemptResync))
        {
            EVERY_N_MILLIS(radioData.beatLength_ms / 8)
            {
